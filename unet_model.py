...
def conv_block(x, num_filters, act=True):
    """
    This function creates a convolutional block. It takes an input tensor x and a number of filters num_filters for the convolutional layer. 
    If act is true (default), it adds batch normalization and ReLU activation after the convolutional layer.
...
def encoder_block(x, num_filters):
    """
    This function creates an encoder block. It takes an input tensor x and a number of filters num_filters for the convolutional layers. 
    The block consists of two convolutional layers followed by max pooling.
...
def unet3plus(input_shape, num_classes=1):
    """
    This function constructs the architecture of a U-Net 3+ model. It takes input_shape specifying the shape of input data and num_classes specifying the number
    of classes for segmentation (default is 1). The model uses a pre-trained ResNet50 base for the encoder and has multiple decoder blocks.
